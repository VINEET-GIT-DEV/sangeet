...
function App() {
  ...
  const [likedSongs, setLikedSongs] = useState(() => {
    const saved = localStorage.getItem('sangeet_likedSongs');
    return saved ? JSON.parse(saved) : [];
  });
  const [playlists, setPlaylists] = useState(() => {
    const saved = localStorage.getItem('sangeet_playlists');
    return saved ? JSON.parse(saved) : [{ id: 1, name: 'My Playlist #1', songs: [] }];
  });
  ...
  useEffect(() => {
    localStorage.setItem('sangeet_likedSongs', JSON.stringify(likedSongs));
  }, [likedSongs]);

  useEffect(() => {
    localStorage.setItem('sangeet_playlists', JSON.stringify(playlists));
  }, [playlists]);
